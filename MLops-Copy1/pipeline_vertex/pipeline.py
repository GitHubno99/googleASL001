...
MAX_TRIAL_COUNT = int(os.getenv("MAX_TRIAL_COUNT", "1010")) #最大試行回数
PARALLEL_TRIAL_COUNT = int(os.getenv("PARALLEL_TRIAL_COUNT", "22")) #同時にいくつまで学習ジョブを走らせるか　(最大試行回数の10%程度が目安）
...
def covertype_train(
    ...
    with dsl.If(
        ...
        r2_score >= r2_score_deployment_threshold, name="deploy__decision"
    ):
