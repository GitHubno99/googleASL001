...
def explain_with_shap(input_data=None, model_name: str = "nw", **kwargs) -> str:
    """
    指定されたモデルの予測結果に対して、SHAPを用いて各変数の影響度をグラフ化します。
    model_name: 'linear', 'nw', 'gauss' のいずれか。
    """
    try:
        # 1. データの安全な辞書化
        raw_data = input_data if input_data else kwargs
        ...
        ordered_features = ["X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8"]
        input_row = [float(raw_data.get(f, 0.0)) for f in ordered_features]
        ...
        # 3. SHAPに渡すための「予測ラッパー関数」（切り替えたエンドポイントを使用）
        def predict_fn(x_array):
            instances = x_array.tolist()
            response = endpoint.predict(instances=instances)
            return np.array(response.predictions)

        # 4. ベースラインとSHAP値の計算 (ここはそのまま)
        background_data = np.array([[0.5, 500.0, 200.0, 150.0, 5.0, 3.0, 0.1, 2.0]])
        ...
        markdown_image = f"![{model_name.upper()}モデルのSHAP解析結果](data:image/png;base64,{img_base64})"
        
        return f"{model_name.upper()}モデルによるSHAP解析が完了しました。\n\n{markdown_image}"

    except Exception as e:
        return f"SHAP解析ツールの実行中にエラーが発生しました: {str(e)}"