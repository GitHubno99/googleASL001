    ...
    pipeline = Pipeline(
        [
            ...
            ("regressor", GaussianProcessRegressor(
                kernel=kernel,
                alpha=alpha,            # 観測ノイズの分散。チューニング対象。
                n_restarts_optimizer=10, # 局所解を避けるための最適化試行回数
                random_state=42 [cite: 101]
            ))
        ]
    )
    ...
    if not hptune:
        ...
        if "gs://" in AIP_MODEL_DIR:
            subprocess.check_call(
                ["gsutil", "cp", MODEL_FILENAME, AIP_MODEL_DIR], stderr=sys.stdout [cite: 105]
            )
